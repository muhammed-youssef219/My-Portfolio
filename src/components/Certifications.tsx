  ...
  return (
    ...
    >
      ...
      <a
        ...
        className="relative flex h-64 items-center justify-center overflow-hidden bg-[var(--surface-2)] p-4"
      >
        <img
      ...
      <div className="relative flex flex-1 flex-col p-4 md:p-5">
        <div className="flex items-center gap-3">
          <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#64748B] to-[#64748B]">
            <Award className="h-4 w-4 text-white" />
          </span>
          <MetaPill>{certificate.date}</MetaPill>
        </div>

        <h3 className="mt-3 text-lg font-bold tracking-[-0.3px] text-[var(--text)]">{certificate.title}</h3>
        <p className="mt-1 text-sm text-[var(--text)]/60">{certificate.issuer}</p>
        <p className="mt-3 text-sm text-[var(--text)]/50">{certificate.detail}</p>
      </div>
    </motion.div>
  )
}
...
export default function Certifications() {
  return (
    <section id="certifications" className="relative overflow-hidden bg-[var(--bg)] py-20 md:py-28">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-[-14%] top-[10%] h-[520px] w-[520px] rounded-full bg-[var(--glow-primary)] blur-[120px]" />
        <div className="absolute right-[-16%] bottom-[-10%] h-[520px] w-[520px] rounded-full bg-[var(--glow-secondary)] blur-[120px]" />
      </div>
